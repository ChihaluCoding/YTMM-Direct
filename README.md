# YouTubeMP3modoki Direct

`https://receive.shamimomo.net/YouTubeMP3modoki/` のバックエンド API を Chrome 拡張機能から直接呼び出し、サイト画面を開かずに変換から Chrome ダウンロード登録まで行う Manifest V3 拡張です。
...
- サーバー状態の確認結果は30秒間キャッシュし、短時間の連続変換で `/health` を毎回叩かないようにしています。
...
権利を持つコンテンツ、または利用許諾のあるコンテンツに限って使用してください。
